...
public class ElectricGuitarPage extends BasePage {
    ...
    private List<WebElement> price; // prices list of items on page

    @FindBy(xpath = "//div[@class='s-item__detail s-item__detail--primary']//span[contains(text(),'shipping')]")
    private List<WebElement> shipping; // shipping prices list of items on page
    ...
    public List<Double> getPrice() {
       ...
       // get item price string and convert it to double price, e.g. "$1,000.00" to "1000.00"
    ...
    // create total (item + shipping) price list
    public List<Double> getPriceAndShippingSum(){
        ...
        for(int i = 0; i < getPrice().size(); ++i){
            priceAndShippingList.add(i, Math.round((price.get(i) + shipping.get(i)) * 100.0) / 100.0); // ×100 and ÷100 to leave two digits after comma
        }
